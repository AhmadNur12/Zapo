...
Typed convenience helpers for the common operations (setChatMute, setChatPin, …).
A generic set / remove that works against any registered app-state schema — for everything without a dedicated helper.
...
WhatsApp passkey & Shortcake linking

An independent reverse-engineering of WhatsApp’s passkey-gated device linking — the Shortcake / CRSC protocol, its cryptography, a correlated mobile ⇄ web wire capture, and why there is no headless bypass.
...
primary_ephemeral_identity received

code confirmed / handoff

companion registered (ADV)
...
s.whatsapp.net
Web (companion)
navigator.credentials.get() — passkey prompt
auto-confirm (handoff proof valid)
skip code-matching (handoff)
iq/pair-device (+ challenges: friction, passkey-create)
retry-with-method: shortcake-with-passkeys (no qr-code)
notification passkey_prologue_request (+ options)
iq/passkey_prologue (credential_id + webauthn_assertion + prologue_payload + handoff_proof)
notification passkey_prologue (prologue_payload + handoff_proof only)
iq/primary_ephemeral_identity
notification crsc_continuation: primary_ephemeral_identity
iq/companion_nonce
notification crsc_continuation: companion_nonce
iq/encrypted_pairing_request
notification crsc_continuation: encrypted_pairing_request
iq/pair-device (2nd) — device registered
iq/pair-success
iq/pair-device-sign
success — linked
​
